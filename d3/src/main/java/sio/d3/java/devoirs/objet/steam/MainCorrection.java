...
public class MainCorrection {
    ...
    public int min_key(List<Float> L) {
        int min = 0;
        float val_min = L.get(0);

        for (int i = 1; i < L.size(); i++) {
            if(val_min>L.get(i)){ val_min = L.get(i); min=i; }
        }
    ...
    public int max_key(List<Float> L) {
        int max = L.size() - 1;
        float val_max=L.get(0);

        for (int i = 1; i < L.size(); i++) {
            if(val_max<L.get(i)){ val_max = L.get(i); max=i; }
        }
    ...
    public float moyenne(List<Float> L) {
        float moy = 0;
